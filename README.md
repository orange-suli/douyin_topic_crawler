...
本项目提供了全自动一键启动脚本，会自动完成虚拟环境创建、依赖安装及浏览器内核配置。

### 1. 使用一键脚本 (推荐)

- **Windows 用户**：直接双击根目录下的 **`start_windows.bat`**。
- **Mac / Linux 用户**：在终端执行 `bash start_mac_linux.sh`。

> [!IMPORTANT]
> **环境要求**：请确保您的电脑已安装 **Python 3.8+**。
> **报错提示**：如果脚本提示 `python` 不是内部或外部命令，请检查安装 Python 时是否勾选了 **"Add Python to PATH"**（将其添加到系统环境变量）。

### 2. 进入看板

脚本启动成功后，会自动检测服务器状态并为您打开浏览器。
- **访问地址**：`http://localhost:8000`
- **默认操作**：输入关键词（如 `AI`），点击「开始抓取」，程序即刻开始工作。
...
## 📊 数据看板展示 (Dashboard Preview)

![抖音多维度数据分析看板](docs/dashboard_preview.png)

访问 `http://localhost:8000` 即可进入全功能的数据分析看板。看板共包含 **5 个 ECharts 图表面板**和 **1 个作品详情列表**：

### 🎛️ 控制面板
- **关键词输入框**：输入任意抖音搜索关键词
- **条数限制**：设置最多抓取条数（建议 5–20）
- **🚀 开始抓取**：触发全链路数据流水线（Playwright → 清洗 → 入库 → 刷新图表）
- **📥 下载 Excel 报表**：一键导出当前关键词的结构化数据（抓取完成后自动激活）

### 📈 五大图表面板

| 面板 | 类型 | 业务价值 |
|---|---|---|
| **核心互动数据对比图** | 柱状图 | 直观对比点赞/评论/分享/收藏的总量差异 |
| **热门标签分布词云** | 词云图 | 一眼识别该话题下最高频的内容标签 |
| **加权爆款得分榜 TOP 10** | 横向条形图 | 基于加权算法（点赞×1/收藏×2/评论×5/转发×10）识别真正的高粘性爆款 |
| **高潜话题生态图谱** | 力导向网络图 | 通过标签共现矩阵可视化创作者的「多标签组合策略」，发现隐性流量密码 |
| **粉丝数与互动转化表现散点图** | 散点图 | 揭示粉丝数量与互动转化率之间的关系，辅助投放 ROI 决策 |

### 📋 作品详情列表
每条视频展示标题（可跳转）、话题标签、互动数据、博主昵称和粉丝数，一览无余。
